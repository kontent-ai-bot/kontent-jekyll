...
describe Jekyll::Kentico do
  ...
  describe 'resolvers' do
    it 'content resolver' do
      visit('resolved_content')
      expect(page).to have_content('Resolved content')
    end

    it 'filename resolver' do
      visit('resolved_filename')
      expect(page).to have_content('Original content')
    end

    it 'front matter resolver' do
      visit('resolved_front_matter')
      expect(page).to have_content('Content from extra front matter')
    end

    it 'data resolver' do
      visit('resolved_data')
      expect(page).to have_content('Content from resolved data')
    end

    it 'custom site processor' do
      visit('custom_test_page')
      expect(page).to have_content('Test page content: 2019-12-12T16:29:11+0000')
    end
  end
end